...
class MARCSModel:
    def __init__(self, file):
        ...
        self.alpha_convection = float(
            lines_marcs_model[current_line].strip().split()[0]
        )
        ...
        for i in range(9):
            ...
            for j in range(10):
                ...
                element_abundance = float(
                    lines_marcs_model[current_line].strip().split()[j]
                )
        ...
        data = np.loadtxt(
            lines_marcs_model[current_line : current_line + self.number_depth_points],
            skiprows=0,
        )
    ...
    def interpolate_all_parameters(self, number_depth_points: int) -> None:
        ...
        self.temperature = interp1d(self.depth, self.temperature, kind="linear")(
            depth_new
        )
    ...
    def interpolate_temperature(
        self, depth_points: Union[list, np.ndarray, int]
        ...
        if isinstance(depth_points, int):
            (
                depth_new,
                temperature_new,
            ) = self._interpolate_temperature_number_depth_points(depth_points)
        elif isinstance(depth_points, list) or isinstance(depth_points, np.ndarray):
            depth_new, temperature_new = self._interpolate_temperature_new_depth(
                depth_points
            )
        else:
    ...
    def _interpolate_temperature_number_depth_points(
        self, number_depth_points: int
    ) -> tuple[np.ndarray, np.ndarray]:
    ...
    def _interpolate_temperature_new_depth(
        self, depth_new: Union[list, np.ndarray]
    ) -> tuple[np.ndarray, np.ndarray]:
        # down or upsample to the number of depth points. returns the interpolated depth and temperature
        # number_depth_points: number of depth points to interpolate to
        ...
        temperature_new = interp1d(self.depth, self.temperature, kind="linear")(
            depth_new
        )
        return depth_new, temperature_new

    def write_m3d_type_model(
        self, new_file: str, depth_points: Union[list, np.ndarray, int]
    ) -> None:
        ...
        with open(new_file, "w") as f:
            ...
            f.write(
                f"* Depth_[cm] Temperature_[K] Electron_pressure_[cgs] Density_[cgs] Microturbulence_[km/s]\n"
            )
        # interpolate to new depth points
        (
            depth_new,
            temperature_new,
            electron_pressure_new,
            density_new,
            vmicro_new,
        ) = self.interpolate_m3d_type_model(depth_points)
        # write data
        with open(new_file, "a") as f:
            for i in range(depth_points):
                f.write(
                    f"{depth_new[i]:>13.6e} {temperature_new[i]:>8.1f} {electron_pressure_new[i]:>12.4E} {density_new[i]:>12.4E} {vmicro_new[i]:>3.1f}\n"
                )

    def interpolate_m3d_type_model(
        self, depth_points: Union[list, np.ndarray, int]
        ...
        if isinstance(depth_points, int):
            (
                ...
                density_new,
                vmicro_new,
            ) = self._interpolate_m3d_number_depth_points(depth_points)
        elif isinstance(depth_points, list) or isinstance(depth_points, np.ndarray):
            (
                depth_new,
                temperature_new,
                ...
                density_new,
                vmicro_new,
            ) = self._interpolate_m3d_new_depth(depth_points)
        else:
            raise TypeError("number_depth_points must be int or list")
        return (
            depth_new,
    ...
    def _interpolate_m3d_number_depth_points(
        self, number_depth_points: int
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        # down or upsample to the number of depth points. returns the interpolated depth and temperature
        # number_depth_points: number of depth points to interpolate to
    ...
    def _interpolate_m3d_new_depth(
        self, depth_new: Union[list, np.ndarray]
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        # down or upsample to the number of depth points. returns the interpolated depth and temperature
        # number_depth_points: number of depth points to interpolate to
...
if __name__ == "__main__":
    ...
    sunint_temperature, sunint_depth = np.loadtxt(
        sunint_file, unpack=True, usecols=(1, 5), dtype=float, skiprows=1, comments="/"
    )
    sunint_depth = marcs_model.radius - sunint_depth

    marcs_50_depth, marcs_50_temperature = np.loadtxt(
        "/Users/storm/PycharmProjects/3d_nlte_stuff/m3dis_l/m3dis/experiments/Multi3D/input_multi3d/atmos/atmos.sun_MARCS_50",
        unpack=True,
        usecols=(0, 1),
        dtype=float,
        skiprows=2,
    ...
    plt.plot(
        sunint_depth, sunint_temperature, label="MARCS TS interpolated", color="black"
    )
